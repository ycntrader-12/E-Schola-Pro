...
def seed_default_tasks_if_empty(db: Session, current_user: User):
    ...
    if count == 0:
        admin_user = db.query(User).filter(User.role.in_(["admin", "formateur"])).first() or current_user
        demo_tasks = [
            Task(
...
def get_tasks(
    ...
    if is_manager:
        ...
        tasks = db.query(Task).filter(
            (Task.target_role == "all") | 
            (Task.target_role == current_user.role)
        ).order_by(Task.created_at.desc()).all()

    result = []
    for t in tasks:
        creator = db.query(User).filter(User.id == t.assigned_by_id).first()
        sub_count = db.query(TaskSubmission).filter(TaskSubmission.task_id == t.id).count()
        
        my_sub = None
        if not is_manager:
            my_sub_obj = db.query(TaskSubmission).filter(
...
def get_task_submissions(
    ...
    for s in submissions:
        ...
        res.append(TaskSubmissionResponse(
            id=s.id,
            task_id=s.task_id,
            user_id=s.user_id,
...
def grade_submission(
    ...
    sub_obj = db.query(TaskSubmission).filter(TaskSubmission.id == submission_id).first()
    if not sub_obj:
