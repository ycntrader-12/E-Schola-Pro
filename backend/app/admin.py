...
class CourseVideoAdmin(ModelView, model=CourseVideo):
    column_list = [CourseVideo.id, CourseVideo.course_id, CourseVideo.title, CourseVideo.order_index]
...
class ClassroomAdmin(ModelView, model=Classroom):
    column_list = [Classroom.id, Classroom.room_id, Classroom.title, Classroom.instructor_id, Classroom.is_active, Classroom.created_at]
...
class MessageAdmin(ModelView, model=Message):
    column_list = [Message.id, Message.sender_id, Message.recipient_id, Message.subject, Message.is_read, Message.created_at]
...
class EventAdmin(ModelView, model=Event):
    column_list = [Event.id, Event.title, Event.start_time, Event.end_time, Event.target_roles]
...
class EventDeliverableAdmin(ModelView, model=EventDeliverable):
    column_list = [EventDeliverable.id, EventDeliverable.event_id, EventDeliverable.user_id, EventDeliverable.submitted_at]
    name = "Livrable Événement"
    name_plural = "Livrables Événements"
    icon = "fa-solid fa-file-arrow-up"

class QuizAdmin(ModelView, model=Quiz):
    column_list = [Quiz.id, Quiz.title, Quiz.created_by_id, Quiz.target_roles, Quiz.time_limit_minutes, Quiz.created_at]
...
class QuizQuestionAdmin(ModelView, model=QuizQuestion):
    column_list = [QuizQuestion.id, QuizQuestion.quiz_id, QuizQuestion.question_text, QuizQuestion.points]
    name = "Question Quiz"
    name_plural = "Questions Quiz"
    icon = "fa-solid fa-circle-question"

class QuizAttemptAdmin(ModelView, model=QuizAttempt):
    column_list = [QuizAttempt.id, QuizAttempt.quiz_id, QuizAttempt.user_id, QuizAttempt.score, QuizAttempt.max_score, QuizAttempt.percentage, QuizAttempt.completed_at]
    name = "Tentative Quiz"
    name_plural = "Tentatives Quiz"
    icon = "fa-solid fa-chart-simple"

class AttendanceAdmin(ModelView, model=Attendance):
    column_list = [Attendance.id, Attendance.user_id, Attendance.date, Attendance.status, Attendance.session_name, Attendance.marked_by_id]
...
class GroupMemberAdmin(ModelView, model=GroupMember):
    column_list = [GroupMember.id, GroupMember.group_id, GroupMember.user_id, GroupMember.joined_at]
