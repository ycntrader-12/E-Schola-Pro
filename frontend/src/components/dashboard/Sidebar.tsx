      ...
      <aside className={`w-64 h-screen fixed left-0 top-0 bg-[#16325c] border-r border-[#1e3a66] text-white flex flex-col py-6 z-50 transition-transform duration-300 shadow-xl lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={`group flex items-center justify-between px-3.5 py-2.5 rounded-xl transition-all font-medium text-sm ${isActive
              ...
              >
                <div className="flex items-center gap-3">
                  <Icon
                    size={19}
                    className={`transition-colors ${isActive ? 'text-white' : 'text-blue-200/80 group-hover:text-white'
                      }`}
                  />
                  <span>{item.name}</span>
                </div>
        ...
        <div className="px-3 shrink-0">
          <Link
            href="/profile"
            onClick={() => setIsOpen && setIsOpen(false)}
            className={`w-full group flex items-center gap-3 px-3.5 py-2.5 rounded-xl transition-all text-sm font-medium ${isSettingsActive
                ? 'bg-[#2563eb] text-white font-bold shadow-md shadow-blue-600/30'
                : 'text-slate-200 hover:bg-white/10 hover:text-white'
              }`}
          >
            <Settings
              size={19}
              className={`transition-colors ${isSettingsActive ? 'text-white' : 'text-blue-200/80 group-hover:text-white'
                }`}
            />
